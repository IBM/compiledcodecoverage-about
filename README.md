...
1. Using Remote Debug Service:
   ...
   - Create a Zowe profile for Z Open Debug that configures both the debug profile service and remote debug service connectivity. See the [sample configuration](https://www.ibm.com/docs/en/developer-for-zos/17.0.x?topic=code-setting-up-z-open-debug#opendebug_connection__define_zowe_connection__title__1).
     <br>_Note:_ When connecting to the z/OS Debugger Profile service from the Zowe Explorer view, the connection to the Code Coverage Service will be established automatically.
...
Coverage results in both the **Coverage Results View** and the **CC Report** are color-coded into three tiers:

| Color     | Icon      | Meaning                                         |
| --------- | --------- | ----------------------------------------------- |
| 🟢 Green  | ✓ check   | Coverage is at or above the green threshold     |
| 🟡 Yellow | ⚠ warning | Coverage is at or above yellow, but below green |
| 🔴 Red    | ✗ close   | Coverage is below the yellow threshold          |

**Threshold configuration** — thresholds are driven by VS Code's built-in [`testing.coverageBarThresholds`](vscode://settings/testing.coverageBarThresholds) setting.
The defaults are `{ red: 0, yellow: 60, green: 90 }`. You can override them in your `settings.json`:
...
Changing this setting while the tree view or a CC Report is open updates the colors immediately — no reload required.

**Color customization** — icon and bar colors follow the `charts.green`, `charts.yellow`, and `charts.red` VS Code color tokens (the same tokens used by VS Code's built-in Test Coverage view). Override them in `workbench.colorCustomizations`:
...
All color tokens are optional — your active VS Code theme provides the defaults automatically.
